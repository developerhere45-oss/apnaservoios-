...
final class APIClient {
    ...
    func acceptBooking(_ bookingId: String, arrivalEstimateMinutes: Int = 30, token: String) async throws -> PartnerBooking {
        let eta = min(max(arrivalEstimateMinutes, 10), 1_440)
        let envelope: BookingEnvelope = try await request(
            path: "/bookings/\(bookingId)/accept",
            method: "POST",
            token: token,
            body: [
                "arrivalEstimateMinutes": eta,
                "arrivalEstimateLabel": "\(eta) minutes"
            ]
        )
