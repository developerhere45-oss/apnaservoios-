    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("ApnaServo keeps profile, address, booking and support details only for service fulfilment. This iOS build uses mock frontend data.")
                    .font(.system(size: 13))
                    .foregroundStyle(AppTheme.muted)
                Text("Terms")
                    .font(.system(size: 17, weight: .bold))
                Text("Final amount is confirmed after inspection. No upfront payment is collected in the mocked user flow.")
                    .font(.system(size: 13))
                    .foregroundStyle(AppTheme.muted)
                Text("Delete Account")
                    .font(.system(size: 17, weight: .bold))
